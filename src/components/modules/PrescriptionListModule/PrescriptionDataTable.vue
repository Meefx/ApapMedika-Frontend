...
import { prescriptionStatusValues } from '@/interfaces/prescription.interface'
...
watch([status], async () => {
  toast.promise(
    usePrescriptionStore().getPrescriptions({
      status: status.value
    }),
    {
      loading: 'Loading prescriptions...',
      success: 'Prescriptions loaded',
      error: 'Error loading prescriptions',
    },
  )
})
    ...
    <div class="flex items-center py-4">
      ...
      <Select v-model="status">
        ...
        <SelectContent>
          <SelectGroup>
            <SelectItem
              v-for="(status, index) in prescriptionStatusValues"
              :key="index"
              :value="status"
              >{{ status }}</SelectItem
            >
